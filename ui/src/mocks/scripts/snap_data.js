...
async function run() {
  ...
  const keyWaypoints = [
    ...
    [-116.61, 31.854], // Segunda/Rayón
    [-116.602, 31.848], // Reforma/Diamante
    [-116.58, 31.78], // Terminal Chapultepec
    [-116.59, 31.785], // Pedro Loyola/Entrada Chapultepec
  ...
  try {
    ...
    if (data.code === 'Ok' && data.routes && data.routes.length > 0) {
      ...
      console.log(
        `Clean route generated successfully. Waypoint count: ${routeGeom.coordinates.length}`,
      )
    } else {
  ...
  for (const stop of stops) {
    ...
    console.log(
      `Snapped Stop #${stop.id} "${stop.name}": [${originalCoords}] -> [${snappedCoords}]`,
    )
  ...
  const stopsInsertRows = snappedStops
    .map((s) => {
