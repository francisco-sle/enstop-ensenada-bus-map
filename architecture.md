...
- **`ui/`**: Houses the single-page React application built with Vite, TypeScript, and Tailwind CSS v4.x.
  ...
  - **`ui/src/components/`**: Organized by feature slice (e.g., Map, Routing, RouteDetail). Encapsulates UI components and domain-specific logic.
...
7. **Map Rendering Loop**: The map components consume state from both the Map Store and TanStack Query cache. Leaflet layers composite OSM base tiles with dynamic route polylines and stop markers, automatically updating based on visibility filters and selection states.
