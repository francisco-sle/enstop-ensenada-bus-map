...
- **`supabase/`**: Contains all backend configurations, including Postgres/PostGIS schema migrations, Row-Level Security (RLS) policies, spatial RPC functions, and Deno Edge Functions (e.g., `route-proxy`). This folder handles the complete data layer and edge compute.
- **`ui/`**: Houses the progressive web application (PWA) built with React, Vite, TypeScript, and Tailwind CSS.
...
- **Global Client State (Zustand v5)**:
  - **Map Store**: Manages viewport state (center/zoom), selected entities, and visibility filters. State is isolated or reset appropriately when navigating between pages.
...
3. **Geometry Degradation Pipeline**: To protect high-fidelity geospatial data, route geometry is fetched through a dual-mode strategy:
   ...
   - Geometry data is proxied through an Edge Function (`route-proxy`) that validates the Turnstile token, invokes a `SECURITY DEFINER` RPC to simplify the geometry (`ST_SimplifyPreserveTopology`), and returns the degraded payload. RLS prevents direct access to the raw geometry table by unprivileged users.
